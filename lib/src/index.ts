...
interface IMermaidPluginOptions {
  /**
   * A mapping of mermaid shortcodes to their corresponding Unicode characters.
   * This allows for easy customization of the mermaid representation.
   */
  mermaids?: Record<string, string>;
}
...
export const mermaidPlugin: (options?: IMermaidPluginOptions) => IPlugin = options => {
  ...
  return {
    ...
    inline: async (
      ...
      if (node.type === "") {
        /**
         * TODO: Add logic to handle specific inline node types (e.g., mermaid).
     ...
     */
    preprocess: tree => {
      // TODO: Modify or traverse the MDAST tree if needed (e.g., convert mermaid syntax)
    },
  };
};
